...
import { EmployeeType, UserType } from '../../../shared/models/rbac.constants';
...
export class DefaultHeaderComponent extends HeaderComponent {
  ...
  readonly usersLinkRoles: (UserType | EmployeeType)[] = [UserType.USER, EmployeeType.ADMIN_STORE];

  canViewLink(roles: (UserType | EmployeeType)[]): boolean {
    const userType = this.#authService.getUserType();
    if (userType === 'user') {
      return roles.includes(UserType.USER);
    }
   ...
   * Returns employee name if employee is logged in, user name if user is logged in
   */
  getLoggedInUserName(): string {
    ...
    // For users, we'll use a generic name since there's no direct getter
    return 'User';
  }
   ...
   * For employees: Resets PIN verification and redirects to verify-pin page
   * For users: Logs out directly (no PIN required)
   */
  onLockAccount(): void {
    const userType = this.#authService.getUserType();
  ...
  /**
   * Logout - triggered by user clicking "Logout"
